---
{
  ...
  "authority_source": "external:project-owner-acceptance-pending — staged at the F2 boundary under TASK-0005; acceptance constitutes the GATE-0004 approval.",
  ...
  "successor": null,
  "limitations": [
    "Disclosed deferrals: telemetry baseline arrives with the engine-api application (F3 entry); feature flags at first need; production signing-key custody and CI-side restore drills are O1 concerns; F3 kernel tables adopt the same RLS/append-only pattern and re-run VS0 at full product scope.",
