---
name: storyworld-fixture-authoring
description: Create rights-safe Storyworld golden, adversarial, mutation, replacement, and cross-media fixtures with explicit invariants and expected findings.
---
...
Turn architectural claims into reproducible evidence without using unlicensed or sensitive media.

## Use this skill when

- A contract, provider adapter, evaluation layer, POC, external-tool round trip, runtime compiler, or Studio flow needs proof

## Do not use this skill when

- The example has no stated invariant or expected result
- The source media cannot be redistributed or used for the planned evaluation
...
1. Choose the smallest synthetic or owner-authorized material that isolates one semantic
2. Declare source rights, exact versions, accepted specifications, permissible variation, and intentional difference
3. Create a valid case and at least one intentionally invalid or adversarial case
4. Inject one defect at a time where possible
5. Define expected deterministic result, model-assisted finding, and required human judgment
6. Record hashes, generation method, dependencies, and redistribution terms
7. Register the fixture and add regression tests

## Required invariants

- Fixtures never imply legal clearance beyond their recorded rights
- One-variable pairs are preferred
- Expected failures must fail for the intended reason
- No model response is treated as ground truth without a declared baseline
...
- All CON-* contracts with fixture requirements
